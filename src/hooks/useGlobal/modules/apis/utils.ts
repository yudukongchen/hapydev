...
export const getSelfAndChildKeys = (apiDatas: { [key: string]: any }, nodeKey: string) => {
  ...
  const digAllNodes = (treeNode: any) => {
    ...
    if (['folder', 'socket_service'].includes(treeNode.data_type) && isArray(treeNode?.children)) {
...
export const getMaxSort = (apiDatas, parent_id) => {
  const parentList = [];
  Object.values(apiDatas).forEach((item: any) => {
