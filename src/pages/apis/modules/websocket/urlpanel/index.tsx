...
type Props = {
  api_id: string;
  value: WebsocketCollection;
  onChange: (newVal: WebsocketCollection) => void;
  onSave: () => void;
};
...
const UrlPanel: React.FC<Props> = (props) => {
  ...
  const handleStartConnection = () => {
    emitGlobal('PROXYS/WEBSOCKET/startConnection', value);
  };

  const handleCancelConnection = useMemoizedFn(() => {
    emitGlobal('PROXYS/WEBSOCKET/cancelConnection', value);
  });
  const handleCloseConnection = useMemoizedFn(() => {
    emitGlobal('PROXYS/WEBSOCKET/closeConnection', value);
  });
