...
const RightPanel = () => {
  ...
  const opensData = useSelector((store: any) => store?.apis?.opens);
  const tabsList = useSelector((store: any) => store?.apis?.tabs?.list);
  const activeId = useSelector((store: any) => store?.apis?.tabs?.active_id);
