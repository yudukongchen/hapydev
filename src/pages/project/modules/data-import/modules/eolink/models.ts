...
const parseList = (dataStructureList) => {
  const newList = [];
  dataStructureList?.forEach((item) => {
    const newItem = cloneDeep(item);
    newItem.schema = parseSchema(JSON.parse(item?.structureData));
